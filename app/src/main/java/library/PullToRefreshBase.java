...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.handmark.pulltorefresh.library.IPullToRefresh#setLocation(float,
	 * float)
	 */
	...
	public final void setOnRefreshListener(final OnRefreshListener<T> listener) {
		mOnRefreshListener = new OnRefreshListener<T>() {
			@Override
			public void onRefresh(PullToRefreshBase<T> refreshView) {
				if (listener != null) {
					try {
						String label = DateUtils.formatDateTime(MyApplication.getInstance(), System.currentTimeMillis(), DateUtils.FORMAT_SHOW_TIME | DateUtils.FORMAT_SHOW_DATE
								| DateUtils.FORMAT_ABBREV_ALL);
						refreshView.getLoadingLayoutProxy().setLastUpdatedLabel(getContext().getString(R.string.app_list_header_refresh_last_update, label));
					} catch (Exception e) {
					}
	...
	public final void setOnRefreshListener(final OnRefreshListener2<T> listener) {
		mOnRefreshListener2 = new OnRefreshListener2<T>() {
			@Override
	...
	public static interface OnPullEventListener<V extends View> {
		 ...
		 * @param direction
		 *            - One of
		 *            {@link Mode#PULL_FROM_START} or
		 *            {@link Mode#PULL_FROM_END}
